...
class URLSessionHTTPClientTests: XCTestCase {
    ...
    func test_getFromURL_failsOnAllInvalidRepresentationCases() {
        ...
        let nonHTTPURLReponse = URLResponse(url: anyURL(), mimeType: nil, expectedContentLength: 0, textEncodingName: nil)
        let anyHTTPURLResponse = HTTPURLResponse(url: anyURL(), statusCode: 200, httpVersion: nil, headerFields: nil)
        
        XCTAssertNotNil(resultError(data: nil, response: nil, error: nil))
        XCTAssertNotNil(resultError(data: nil, response: nonHTTPURLReponse, error: nil))
        ...
        XCTAssertNotNil(resultError(data: nil, response: nonHTTPURLReponse, error: anyError))
        XCTAssertNotNil(resultError(data: nil, response: anyHTTPURLResponse, error: anyError))
        XCTAssertNotNil(resultError(data: anyData, response: nonHTTPURLReponse, error: anyError))
        XCTAssertNotNil(resultError(data: anyData, response: anyHTTPURLResponse, error: anyError))
        XCTAssertNotNil(resultError(data: anyData, response: nonHTTPURLReponse, error: nil))
    }
