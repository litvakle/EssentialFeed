...
class CodableFeedStoreTests: XCTestCase {
        ...
        sut.retrieve { firstResult in
            sut.retrieve { secondResult in
                ...
                case (.empty, .empty):
                    break
                default:
                    XCTFail("Expected retrieving twice empty result, got \(firstResult) and \(secondResult) instead")
                }
    ...
    func test_retrieveAfterInsertingToEmptyCache_deliversInsetedValues() {
        let sut = makeSUT()
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            XCTAssertNil(insertionError, "Expected feed to be inseted successfully")
            exp.fulfill()
        }
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    ...
                    default:
                        XCTFail("Expected retrieving twice from non empty cache to deliver same found result with feed \(feed) and timestamp \(timestamp). Gor \(firstResult) and \(secondResult) instead")
                    }
